...
class SoModel extends CI_Model
{
  ...
  public function getOrderHeaderrs($username,$noso){
    ...
    $soh=array();
    //$ukur=array();
    $this->db->select('B.perusahaan,B.alamat,B.tel,A.*, format(SUM(D.qty * D.ukuran * D.price),2) + 0 AS total ,IF(A.ppn=0,0,format(SUM(D.qty * D.ukuran * D.price) * 10/100),2) + 0 AS nilaippn');
